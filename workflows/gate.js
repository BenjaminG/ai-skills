export const meta = {
  name: 'gate',
  description: 'Deterministic quality gate: parallel reviewers + adversarial verify + context check',
  whenToUse: 'Run on a feature branch before merge to surface BLOCKER/MAJOR/NIT findings, refuted by independent skeptics and cross-checked against project rules.',
  phases: [
    { title: 'Review' },
    { title: 'Verify' },
    { title: 'Context' },
...
const FINDING_REQUIRED = ['rule_id', 'file', 'line', 'location', 'tier', 'message', 'evidence', 'suggested_fix'];
    ...
    annotations: {
      ...
      items: {
        ...
        properties: {
          ...
          source: { type: 'string', enum: ['linear', 'pr', 'session', 'claude-md', 'adr', 'none'] },
...
const reviewerResults = await pipeline(
  enabledReviewers,
  // Stage 1: review
  r => agent(buildReviewPrompt(args), {
