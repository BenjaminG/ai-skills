---
name: naboo-pr-risk-score
description: Score one Naboo pull request in read-only shadow mode.
...
Score one open `naboo-team/naboo` pull request. Return advice in chat only: never
approve, review, comment, edit, merge, or start a workflow.
...
SHAs. Otherwise report, in the user's language:

```text
PR #<number> @ <sha12>
Score: <score>/100
Verdict: LOW_RISK_CANDIDATE | HUMAN_REVIEW_REQUIRED | INCONCLUSIVE
Dimensions: <five values>
Failed gates: <id: reason, or none>
Evidence: <file: reason>
Unknowns: <items, or none>
```

`LOW_RISK_CANDIDATE` is advice, never an approval. A low score cannot override a
failed gate. Delete only the temporary directory created for this run.

**Done when:** the reported SHA is still current, the result is shown in chat, no
GitHub write occurred, and the temporary directory is removed.
