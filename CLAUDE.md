...
- **Review & PR** (`pr-create`, `pr-challenge`, `pr-feedback`, `pr-respond`, `pr-comment`, `fixup`, `ci-watch`, `babysit-prs`, `pr-dash`, `pr-explain`, `review-mining`, `qa-plan`, `qa-run`, `consensus`): Git/GitHub and review automation. `pr-challenge` reviews someone else's PR as a colleague would (questions, not findings) and hands its drafts to `pr-comment`, which also posts `gate-wf` findings. `pr-dash` owns the scan service (`pr-scan.py`) that both it and `babysit-prs` read; neither needs the other. `pr-explain` opens a herdr tab where a fresh session explains a `held` thread in plain words.
